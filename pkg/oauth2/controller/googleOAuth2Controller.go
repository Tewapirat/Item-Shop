...
import (
	...
	_oauth2Model "github.com/TewApirat/items-shop-api/pkg/oauth2/model"
...
func (c *googleOAuth2Controller)PlayerLoginCallback(pctx echo.Context) error {
	...
	if err != nil {
		c.logger.Errorf("Failed to ecchange token : %s",err.Error())
...
func (c * googleOAuth2Controller)Logout(pctx echo.Context)  error {
	...
	if err := c.revokToken(accessToken.Value);err != nil {
...
func (c *googleOAuth2Controller)revokToken(accessToken string) error {
...
func (c *googleOAuth2Controller) callbackValidating(pctx echo.Context)error{
	...
	//https://asdadsasd/callback?code=asd198sda&state=1231321231

	state := pctx.QueryParam("state")

	stateFromCookie, err := pctx.Cookie(stateCookieName)
	if err != nil {
		c.logger.Errorf("Failed to get state form cookie: %s",err)
...
func (c *googleOAuth2Controller)getUserInfo(client *http.Client)(*_oauth2Model.UserInfo, error){
	...
	if err := json.Unmarshal(userInfoInBytes, &userInfo); err != nil {
		c.logger.Errorf("Error unmarshlling user info: %s", err.Error())
		return nil, err
